...
enum PDBErrors {
    ...
    EC_MINI_PDB,                    // @@@ not found, guessing it goes here
    EC_MAX
};

// @@@ error strings for dbg
const wchar_t * const szPDBErrors[] = {
    L"EC_OK",                          // no problem
    L"EC_USAGE",                       // invalid parameter or call order
    L"EC_OUT_OF_MEMORY",               // out of heap
    L"EC_FILE_SYSTEM",                 // "pdb name", can't write file, out of disk, etc.
    L"EC_NOT_FOUND",                   // "pdb name", PDB file not found
    L"EC_INVALID_SIG",                 // "pdb name", PDB::OpenValidate() and its clients only
    L"EC_INVALID_AGE",                 // "pdb name", PDB::OpenValidate() and its clients only
    L"EC_PRECOMP_REQUIRED",            // "obj name", Mod::AddTypes() only
    L"EC_OUT_OF_TI",                   // "pdb name", TPI::QueryTiForCVRecord() only
    L"EC_NOT_IMPLEMENTED",             // -
    //L"EC_V1_PDB",                      // "pdb name", PDB::Open* only (obsolete)
    L"EC_UNKNOWN_FORMAT",
    L"EC_FORMAT",                      // accessing pdb with obsolete format
    L"EC_LIMIT",
    L"EC_CORRUPT",                     // cv info corrupt, recompile mod
    L"EC_TI16",                        // no 16-bit type interface present
    L"EC_ACCESS_DENIED",               // "pdb name", PDB file read-only
    L"EC_ILLEGAL_TYPE_EDIT",           // trying to edit types in read-only mode
    L"EC_INVALID_EXECUTABLE",          // not recogized as a valid executable
    L"EC_DBG_NOT_FOUND",               // A required .DBG file was not found
    L"EC_NO_DEBUG_INFO",               // No recognized debug info found
    L"EC_INVALID_EXE_TIMESTAMP",       // Invalid timestamp on Openvalidate of exe
    L"EC_CORRUPT_TYPEPOOL",            // A corrupted type record was found in a PDB
    L"EC_DEBUG_INFO_NOT_IN_PDB",       // returned by OpenValidateX
    L"EC_RPC",                         // Error occured during RPC
    L"EC_UNKNOWN",                     // Unknown error
    L"EC_BAD_CACHE_PATH",              // bad cache location specified with symsrv
    L"EC_CACHE_FULL",                  // symsrv cache is full
    L"EC_TOO_MANY_MOD_ADDTYPE",        // Addtype is called more then once per mod
    L"EC_MINI_PDB",                    // @@@ not found, guessing it goes here
    L"EC_MAX"
};
...
enum { 
    ...
    copyCustomModSyms       = 0x00000020,   // @@@ guessed it
};
...
PdbInterface PDB {                 // program database
    ...
    //
    virtual BOOL OpenSrc(OUT Src** ppsrc) { return false; } //@@@
